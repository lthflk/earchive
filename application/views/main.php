...
?>
      ...
      <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="module-submenu">
          <ul class="nav navbar-nav navbar-right">
			...
			<li><a href="<?php echo site_url('/home/dl').($_SERVER['QUERY_STRING']? '?'.$_SERVER['QUERY_STRING'] : '') ?>"><i class="glyphicon glyphicon-download"></i> Unduh Data</a></li>
          </ul>
        </div><!-- /.navbar-collapse -->
      </div><!-- /.container-fluid -->
    </nav>
		...
		<div class = "input-group">
			...
			<select class="form-control" name="dep" id="dep">
				...
				<?php
					if(isset($dep)) {
						foreach($dep as $p) {
							echo "<option value=\"".$p['id_dep']."\" ".($src['dep']==$p['id_dep']?"selected=selected":"").">"." - ".$p['nama_dep']."</option>";
						}
					}
				?>
			</select>
	...
	<!-- ./panel -->
        ...
        <div class="row" id="hslsrc">
			<table id="tblhslsrc" class="table table-bordered table-hover" cellspacing="0" width="100%">
				...
				<tbody>
					<?php
						...
						foreach($data as $a) {
							if ($a['jenisdok']==1 || $_SESSION['tipe'] != "user"){
								if($a['dep']==$id_depar_user || $_SESSION['tipe'] == "sumin"){
									echo "<tr>";
									echo "<td>".$no."</td>";
									...
									echo "<td>".$a['uraian']."</td>";
									echo "<td>".$a['ket']."</td>";
									// echo "<td>".($a['jenisdok']==1?"Biasa":"Rahasia")."</td>";
									if($a['file']=="") {
										echo "<td></td>";
									}else {
									...
									echo "<td>";
								   if(isset($_SESSION['akses_modul']['entridata']) && $_SESSION['akses_modul']['entridata']=='on' && ($a['jenisdok'] == 1 && $_SESSION['tipe'] == "admin") || $_SESSION['tipe'] == "sumin"){
			...
			<form method="POST" action="<?php echo site_url('/home/set_pagination'); ?>">
			<input name="jumlah_perpage" min="1" style="width: 50px;" type="number" oninvalid="this.setCustomValidity('angka harus sama atau lebih besar dari 1')" oninput="setCustomValidity('')">
			<input type="submit" name="" value="tampil">
			<i>Jumlah data yang ingin ditampilkan</i>
			</form>
        </div>
        <!-- /.row -->
<?php
